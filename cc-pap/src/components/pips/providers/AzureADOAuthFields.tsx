import React from "react";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { AlertCircle, ExternalLink } from "lucide-react";
import { Button } from "@/components/ui/button";

interface AzureADOAuthFieldsProps {
  config: {
    authUrl: string;
    tokenUrl: string;
    clientId: string;
    clientSecret: string;
    scopes: string;
    callbackUrl: string;
  };
  onChange: (config: any) => void;
  tenantId: string;
}

export function AzureADOAuthFields({ config, onChange, tenantId }: AzureADOAuthFieldsProps) {
  const handleFieldChange = (field: string, value: string) => {
    onChange({
      ...config,
      [field]: value
    });
  };

  // Auto-populate URLs based on tenant ID
  React.useEffect(() => {
    if (tenantId && !config.authUrl && !config.tokenUrl) {
      onChange({
        ...config,
        authUrl: `https://login.microsoftonline.com/${tenantId}/oauth2/v2.0/authorize`,
        tokenUrl: `https://login.microsoftonline.com/${tenantId}/oauth2/v2.0/token`
      });
    }
  }, [tenantId, config, onChange]);

  return (
    <div className="space-y-4 p-4 border rounded-lg bg-background border-blue-200 dark:border-blue-800">
      <h5 className="text-sm font-semibold text-blue-800 dark:text-blue-200">Azure AD OAuth 2.0 Configuration</h5>
      
      <div className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="azure-tenant-id" className="text-sm font-medium">Tenant ID</Label>
          <Input 
            id="azure-tenant-id" 
            placeholder="12345678-1234-1234-1234-123456789012"
            value={tenantId}
            onChange={(e) => {
              const tenant = e.target.value;
              onChange({
                ...config,
                authUrl: `https://login.microsoftonline.com/${tenant}/oauth2/v2.0/authorize`,
                tokenUrl: `https://login.microsoftonline.com/${tenant}/oauth2/v2.0/token`
              });
            }}
            className="w-full"
          />
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Find your Tenant ID in Azure Portal → Azure Active Directory → Overview
          </p>
        </div>
        
        <div className="space-y-2">
          <Label htmlFor="azure-auth-url" className="text-sm font-medium">Authorization URL</Label>
          <Input 
            id="azure-auth-url" 
            placeholder="https://login.microsoftonline.com/{tenant}/oauth2/v2.0/authorize"
            value={config.authUrl}
            onChange={(e) => handleFieldChange('authUrl', e.target.value)}
            className="w-full"
          />
        </div>
        
        <div className="space-y-2">
          <Label htmlFor="azure-token-url" className="text-sm font-medium">Token URL</Label>
          <Input 
            id="azure-token-url" 
            placeholder="https://login.microsoftonline.com/{tenant}/oauth2/v2.0/token"
            value={config.tokenUrl}
            onChange={(e) => handleFieldChange('tokenUrl', e.target.value)}
            className="w-full"
          />
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="azure-client-id" className="text-sm font-medium">Application (Client) ID</Label>
            <Input 
              id="azure-client-id" 
              placeholder="Your Azure AD application client ID"
              value={config.clientId}
              onChange={(e) => handleFieldChange('clientId', e.target.value)}
              className="w-full"
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="azure-client-secret" className="text-sm font-medium">Client Secret</Label>
            <Input 
              id="azure-client-secret" 
              type="password"
              placeholder="Your Azure AD application client secret"
              value={config.clientSecret}
              onChange={(e) => handleFieldChange('clientSecret', e.target.value)}
              className="w-full"
            />
          </div>
        </div>
        
        <div className="space-y-2">
          <Label htmlFor="azure-scopes" className="text-sm font-medium">Scopes</Label>
          <Input 
            id="azure-scopes" 
            placeholder="openid profile email User.Read GroupMember.Read.All"
            value={config.scopes}
            onChange={(e) => handleFieldChange('scopes', e.target.value)}
            className="w-full"
          />
          <p className="text-xs text-gray-500">
            Required: openid, profile, email. Optional: User.Read, GroupMember.Read.All
          </p>
        </div>
        
        <Alert className="border-blue-200 bg-blue-50">
          <AlertCircle className="h-4 w-4 text-blue-600" />
          <AlertDescription className="text-blue-800">
            <div className="space-y-2">
              <p>Configure your Azure AD application with:</p>
              <div className="bg-blue-100 p-2 rounded text-sm">
                <p><strong>Redirect URI:</strong> <code className="bg-blue-200 px-1 rounded">http://localhost:8000/pip/oauth/callback/azure_ad</code></p>
                <p><strong>API Permissions:</strong> Microsoft Graph - User.Read, GroupMember.Read.All</p>
                <p><strong>Authentication:</strong> Allow public client flows: No</p>
              </div>
              <Button 
                variant="outline" 
                size="sm" 
                className="mt-2"
                onClick={() => window.open('https://docs.microsoft.com/en-us/azure/active-directory/develop/quickstart-register-app', '_blank')}
              >
                <ExternalLink className="h-3 w-3 mr-1" />
                Azure AD App Registration Guide
              </Button>
            </div>
          </AlertDescription>
        </Alert>
      </div>
    </div>
  );
}
